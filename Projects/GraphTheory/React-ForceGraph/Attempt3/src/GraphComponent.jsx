        ...
        <div>
            <ForceGraph2D
                ...
                nodeCanvasObject={(node, ctx, globalScale) => {
                    ...
                    let nodeColor = node.color; // default color
                    if (node === hoverNode) {
                        nodeColor = 'red'; // color when hovered
                    }
                    if (node === selectedNode) {
                        nodeColor = 'blue'; // color when selected
                    }

                    // Draw the node
                    const size = node === hoverNode ? 12 : 8; // larger size when hovered
                    if (node === selectedNode) {
                        size = 15; // even larger size when selected
                    }
                    ctx.beginPath();
                    ...
                    // Draw the label
                    ctx.fillStyle = 'black';
                    ctx.fillText(label, node.x, node.y + size + 4); // adjust label positioning
                }}
            />
        </div>
    );
